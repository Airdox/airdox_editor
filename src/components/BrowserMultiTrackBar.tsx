...
export const BrowserMultiTrackBar: React.FC<BrowserBarProps> = ({
  ...
  return (
    <div className="bg-[#0b0c10] border-t border-[#1a1d26] flex flex-col select-none z-30">
      {/* Strip header with BROWSER angled tab and rekordbox logo */}
      <div className="h-7 bg-[#0f1117] flex items-center justify-between px-3 border-b border-[#1c1f2a]">
        {/* Left: BROWSER angled tab & Pioneer Rekordbox branding */}
        <div className="flex items-center space-x-3">
          {/* Angled polygon tab BROWSER */}
          <button
            onClick={onToggle}
            className="rb-tab-chamfer bg-[#1a1d28] text-neutral-300 hover:text-white text-[10px] font-bold px-4 py-1 tracking-wider uppercase flex items-center space-x-1.5 transition-colors border-r border-[#2d3345]"
          >
            <span>BROWSER</span>
          ...
          {isOpen && (
            <span className="text-[10.5px] text-neutral-400 font-mono pl-3 hidden sm:inline">
              Sammlung: <strong className="text-white">{filteredTracks.length}</strong> Tracks
            </span>
          )}
        </div>
