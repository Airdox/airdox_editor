...
export class PipelineDoubleSeparator implements IStemSeparator {
  ...
  async separate(request: BackendSeparationRequest): Promise<BackendSeparationResult> {
    ...
    const simulated = this.options.simulatedMsPerFrame ?? 0; if (simulated > 0) await new Promise((resolve) => setTimeout(resolve, Math.min(300, simulated * Math.max(1, input.frames / 1000))));
