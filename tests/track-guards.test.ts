...
runTest('T7 origins', 'Rekordbox origins are guarded, others are open', () => {
  same(isRekordboxOrigin(DataOrigin.REKORDBOX_XML), true, 'XML guarded');
  same(isRekordboxOrigin(DataOrigin.REKORDBOX_DB), true, 'DB guarded');
  same(isRekordboxOrigin(DataOrigin.REKORDBOX_ANLZ), true, 'ANLZ guarded');
  same(isRekordboxOrigin(DataOrigin.PROJECT), false, 'Project open');
  same(isRekordboxOrigin(DataOrigin.LOCAL_ANALYSIS), false, 'Local import open');
  same(isRekordboxOrigin(DataOrigin.USER_EDIT), false, 'User edit open');
  same(isRekordboxOrigin(DataOrigin.GENERATED_FALLBACK), false, 'Demo open');
  same(isRekordboxOrigin(undefined), false, 'Undefined open');
});

runTest('T7 origins', 'Waveform source rule: RB tracks are ANLZ-only', () => {
  same(selectDeckWaveformSource(DataOrigin.REKORDBOX_XML), 'ANLZ_ONLY', 'XML');
  same(selectDeckWaveformSource(DataOrigin.REKORDBOX_DB), 'ANLZ_ONLY', 'DB');
  same(selectDeckWaveformSource(DataOrigin.REKORDBOX_ANLZ), 'ANLZ_ONLY', 'ANLZ');
  same(selectDeckWaveformSource(DataOrigin.LOCAL_ANALYSIS), 'LOCAL_ALLOWED', 'Local import');
  same(selectDeckWaveformSource(DataOrigin.PROJECT), 'LOCAL_ALLOWED', 'Project');
  same(selectDeckWaveformSource(undefined), 'LOCAL_ALLOWED', 'Undefined');
});
...
runTest('T7 ppth', 'PPTH plausibility tolerates encoding, flags mismatch', () => {
  ...
  same(ppthMismatchNote(undefined, 'C:\\Music\\Track.wav'), null, 'Missing PPTH never warns');
  same(ppthMismatchNote('/PIONEER/Music/Track.wav', ''), null, 'Missing track path never warns');
});
...
runTest('T7 round-trip', 'Project save/load preserves verbatim nodes and grid origin', () => {
  ...
  same(persisted.beats!.length, 4, 'Nodes persisted');
  same(persisted.origin, DataOrigin.REKORDBOX_ANLZ, 'Grid origin persisted');

  const rebuilt = adoptSerializedGrid(persisted, 240, DataOrigin.REKORDBOX_XML);
  same(rebuilt.origin, DataOrigin.REKORDBOX_ANLZ, 'Grid origin survives (not the track origin)');
  same(rebuilt.beats.length, 4, 'Node count survives');
  for (let i = 0; i < nodes.length; i++) {
    same(rebuilt.beats[i].time, nodes[i].time, `Node ${i} time verbatim`);
    same(rebuilt.beats[i].barNumber, nodes[i].barNumber, `Node ${i} bar`);
    same(rebuilt.beats[i].beatInBar, nodes[i].beatInBar, `Node ${i} beat-in-bar`);
    same(rebuilt.beats[i].bpm, nodes[i].bpm, `Node ${i} per-beat bpm`);
    same(rebuilt.beats[i].index, i, `Node ${i} re-indexed`);
  }
  same(rebuilt.beats[3].tailExtended, true, 'Tail flag survives');
  same(rebuilt.beats[0].tailExtended, undefined, 'Verbatim node stays unflagged');
});
...
runTest('T8 legacy', 'Old Rekordbox projects without nodes remain sparse', () => {
  ...
  same(rebuilt.origin, DataOrigin.REKORDBOX_XML, 'Track origin retained');
  same(rebuilt.firstBeat, 0.5, 'Scalar anchor kept');
  same(rebuilt.beats.length, 0, 'No uniform Rekordbox grid invented');
});
...
runTest('T8 shift', 'Rigid shift preserves intervals and provenance', () => {
  ...
  same(shifted.length, 3, 'All nodes kept');
  same(shifted[0].time, 0.469 + 0.1, 'Shift applied');
  ...
  same(shifted[2].tailExtended, true, 'Tail flag kept');
  same(shifted[1].barNumber, 1, 'Bar number kept');
  same(nodes[0].time, 0.469, 'Input untouched');
});
...
runTest('T8 shift', 'Negative shifts drop pre-zero nodes and re-index', () => {
  ...
  same(shifted.length, 1, 'Pre-zero node dropped');
  same(shifted[0].index, 0, 'Re-indexed');
  same(shifted[0].time, 0.469 - 0.001, 'Survivor shifted');
  same(shifted[0].beatInBar, 2, 'Musical position kept');

  const identity = shiftBeatNodes(nodes, 0);
  same(identity.length, 2, 'Zero delta keeps all');
...
runTest('Bridge payload', 'Offset Uint8Array views copy byte-exactly', () => {
  ...
  same(normalized.byteLength, 40, 'Exact length');
  const bytes = new Uint8Array(normalized);
  for (let i = 0; i < 40; i++) {
    same(bytes[i], (37 + i) % 256, `Byte ${i} preserved`);
  }
});
...
runTest('Bridge payload', 'IPC-shaped ANLZ payload decodes after normalization', () => {
  ...
  same(extraction.waveform!.length, 600, 'DAT buckets intact');
});
...
report('DECK-LOADER & EDIT-GUARD TEST SUITE (T7–T8)');
