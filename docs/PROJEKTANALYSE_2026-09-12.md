...
**A1 — 20 Roh-Konsolenaufrufe am Loggersystem vorbei. ✅ Behoben (12.09.)**
...
*Umgesetzt:* die sechs Zeilen, die nur neben einem bestehenden `logger`-Aufruf standen, sind weg (reine Duplikate); die 15 übrigen sind auf `logger.info/warn/error` umgehängt — mit Kategorie (`DATABASE`, `XML_IMPORT`, `AUDIO_ENGINE`, `SYSTEM`), deutschen Meldungen und strukturierten `details` (Fehlertext, Stack, Warnungslisten). Eine Zeile wurde dabei von `warn` auf `error` hochgestuft, weil der Fall harten Stillstand bedeutet („keine lokale master.db → ein XML-Track kann nicht geladen werden").
*Gesichert:* `tests/source-hygiene.test.ts` H5 verbietet `console.(log|info|warn|error|debug)` in `src/**` (Ausnahme `utils/logger.ts`) — die Regel gilt ab jetzt bei jedem `npm test` auf ubuntu und windows.
...
**C3 — 539 kB JS in einem Chunk plus 2,0 MB Quellmap im Paket (Quellmap jetzt ✅ ausgeschlossen).** `vite.config.ts` setzt `build.sourcemap: true`; electron-builder packt `dist/**/*` → die `.map` wandert ins asar. **Umgesetzt:** `build.files` in package.json schließt `!dist/**/*.map` aus — `dist/` behält die Maps für die lokale Fehlersuche, das installierte Paket wächst nicht mehr um 2 MB und gibt keine Quellen preis. Verbleibender Vorschlag: `build.rollupOptions.output.manualChunks` für `lucide-react`/Vendor; außerdem `codeSplitting` für die Modal-Brocken (`DatabaseExtractionModal` 696 Zeilen, `RekordboxXmlImportModal` 642) per `React.lazy`, weil beide nicht zum Start gehört werden müssen. *Nutzen:* kleineres Setup, schnellerer Start (weniger Parsing vor dem ersten Paint), Quellcode nicht im installierten Paket.
