...
- **Echt verifiziert (10.09.2026, Windows):** Stufe 0 an `D:\PIONEER\Master\master.db` (10.726 Tracks, 95 % mit AnalysisDataPath, Read-Only bewiesen) und Stufe 4 an Track 100011458 (DAT/EXT/2EX, Gate `PPTH+PQTZ+PWV*` PASS, PPTH-Platzhalter `?/` als `PLACEHOLDER_BASENAME` klassifiziert, Quellen byte-identisch).
...
- Stufe 4 – ANLZ-Tag-Inventar: `npm run probe:anlz` (`scripts/anlz-probe.mjs`, läuft über `tsx` mit dem App-Parser) liest DAT/EXT/.2EX read-only und berichtet PPTH + Plausibilitätsvergleich mit `djmdContent`, alle Tags, pro Waveform-Tag `len_entry_bytes`/`len_entries`/Stil, PQTZ-Beats, Cues/Loops und PSSI-Parameter; Gate = `PPTH + PQTZ + PWV*` plus identischem SHA-256-Fingerabdruck. Das Inventar (`tagInventory`) wird read-only in `src/rekordbox/anlzParser.ts` mitgeführt (keine Zweitimplementierung). Test: `npx tsx tests/anlz-probe.test.ts`.
