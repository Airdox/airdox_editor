...
| Befehl | Artefakt |
| --- | --- |
| `npm run package:win:nsis` | Installer `airdox_SMART_Editor-0.4.7-setup.exe` |
| `npm run package:win:portable` | Portable `.exe` (`airdox_SMART_Editor-0.4.7-portable.exe`) |
...
Tag (z. B. `git tag v0.4.7 && git push --tags`) wird automatisch ein GitHub
